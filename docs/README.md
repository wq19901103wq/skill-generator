# Skill Generator 文档中心

欢迎查阅 Skill Generator 的完整文档。

---

## 快速导航

| 文档 | 适合人群 | 内容 |
|------|---------|------|
| [../README.md](../README.md) | 所有用户 | 项目简介、快速开始 |
| [AI_QUICKSTART.md](./AI_QUICKSTART.md) | ⭐ AI Agent / 开发者 | **5分钟上手**、模块速查表、关键禁忌 |
| [MODULE_INDEX.md](./MODULE_INDEX.md) | 开发者 | **按问题查文件**（出错了直接定位） |
| [API.md](./API.md) | 开发者 | 编程接口参考 |
| [TEMPLATE_GUIDE.md](./TEMPLATE_GUIDE.md) | 模板作者 | 如何创建自定义模板 |
| [WIKI_GUIDE.md](./WIKI_GUIDE.md) | Skill 用户 | 如何编辑 Wiki 知识库 |
| [FAQ.md](./FAQ.md) | 所有用户 | 常见问题解答 |
| [LESSONS_LEARNED.md](./LESSONS_LEARNED.md) | 贡献者 | 踩坑记录，避免重复踩坑 |
| [../PROJECT_MAP.md](../PROJECT_MAP.md) | 贡献者 | 项目架构和路线图 |
| [../PROGRESS.md](../PROGRESS.md) | 关注者 | 项目进度和变更记录 |

---

## 按场景选择文档

### 🚀 我是新手，想快速使用

1. 阅读 [../README.md](../README.md) - 了解项目
2. 按照"快速开始"生成第一个 Skill
3. 遇到问题查看 [FAQ.md](./FAQ.md)

### 🎨 我想创建自定义模板

1. 阅读 [TEMPLATE_GUIDE.md](./TEMPLATE_GUIDE.md)
2. 参考现有模板 `templates/personal_digital_twin/`
3. 测试并发布你的模板

### 🔧 我是开发者，想集成或扩展

1. 阅读 [API.md](./API.md) 了解编程接口
2. 查看 [../PROJECT_MAP.md](../PROJECT_MAP.md) 了解架构
3. 提交 PR 贡献代码

### ✏️ 我想手动编辑生成的 Skill

1. 阅读 [WIKI_GUIDE.md](./WIKI_GUIDE.md)
2. 直接编辑 `data/wiki/*.md` 文件
3. 重启 OpenClaw 生效

---

## 文档状态

| 文档 | 状态 | 最后更新 |
|------|------|---------|
| README.md | ✅ 完整 | 2026-04-12 |
| API.md | ✅ 完整 | 2026-04-12 |
| TEMPLATE_GUIDE.md | ✅ 完整 | 2026-04-12 |
| WIKI_GUIDE.md | ✅ 完整 | 2026-04-12 |
| FAQ.md | ✅ 完整 | 2026-04-12 |
| PROJECT_MAP.md | ✅ 完整 | 2026-04-11 |
| PROGRESS.md | ✅ 完整 | 2026-04-12 |

---

## 贡献文档

发现文档问题？欢迎贡献：

1. Fork 项目
2. 编辑文档
3. 提交 PR

---

*文档版本: v3.0*
